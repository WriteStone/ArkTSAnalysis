...
export default class EntryAbility extends UIAbility {
    ...
    onWindowStageCreate(windowStage: window.WindowStage): void {
        ...
        windowClass.setWindowLayoutFullScreen(isLayoutFullScreen).then(() => {
            ...
            hilog.error(this.domainId, this.logTag, `Failed to setWindowLayoutFullScreen. errCode is ${error.code}, errMessage is ${error.message}`);
        });
        windowStage.loadContent('pages/HomePage', (error) => {
            if (error.code) {
                hilog.error(this.domainId, this.logTag, `Failed to load the content. errCode is ${error.code}, errMessage is ${error.message}`);
                return;
            }
